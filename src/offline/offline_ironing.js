...
export default class OfflineIroning extends Component{
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        axios.post(api.U('ironing'),api.data({token:this.props.token}))
        .then(response => {
    ...
    onSearchRequest(word) {
        axios.post(api.U('ironing'),api.data({token:this.props.token,clean_number:word}))
        .then(response => {
                ...
                <section className='ui-container'>
                    <div className='ui-box-between' style={{paddingBottom:'16px'}}>
                        ...
                        <div style={{height:'40px',lineHeight:'40px'}}>
                            ...
                            >全选</span>
                            <input type='button' value='熨烫' className='ui-btn-tab' onClick={this.onConfirmResquest}/>
                        </div>
                    </div>
